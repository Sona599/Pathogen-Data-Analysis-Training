...
```{r read.dat}
~#Test
#  test<-read_csv(/GitHub/Pathogen-Data-Analysis-Training/simulated_data/survey_data_simulated.csv)
...
microbio_clean %>% 
select(household_id, sample_type, tc_mpn:ar_ecoli_mpn_hi) %>% 
  head(10) #view the first 10 rows of the
```
...
microbio_clean <- microbio_clean %>% 
  mutate(
    across(everything(), ~replace_na(.x, 0)), # replace NAs with 0s across all variables in data frame
...
#bonus- check variable frequency by sample
...
microbio_clean2 <- microbio_clean2 %>%
  ...
  mutate(
    across(matches("^adjested_.*_adj$"),~log10(.) +1, .names= "log_{.col}")
  )
...
#add code to add 1to ajusted esbl cfu to shift values from negative to positive
#for MPN  we already did log 10()+1, but for esbl we only did log10()but didnt add 1 , so we had negative values, here we shift values up by 1. 
  microbio_clean2$log_adjusted_esbl_cfu <- microbio_clean2$log_adjusted_esbl_cfu+1
```
